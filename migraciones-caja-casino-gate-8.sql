-- ═════════════════════════════════════════════════════════════════════════════
-- IRIS — Caja: las acciones del operador funcionan también en modo CASINO (P6)
-- Idempotente (create or replace). Correr en el SQL Editor DESPUÉS de toda la
-- serie de caja (… stage6 + migraciones-caja-* hasta la 7).
--
-- PROBLEMA: las 5 funciones de acción del operador abortaban si caja_enabled
-- estaba OFF, sin excepción para casino. En un tenant con casino_deposit_enabled
-- = true el pozo manual está dormido (la fuente de verdad es el casino) y es
-- natural tener caja_enabled OFF; pero entonces sueldo/descarga/cierre quedaban
-- bloqueados, aunque la billetera del operador SÍ se mueve por las verificaciones
-- de carga/pago (que ya bypasean caja_enabled en modo casino, ver src/lib/caja.ts).
--
-- FIX: el guard pasa de `fn_caja_enabled` a `fn_caja_enabled OR fn_casino_enabled`.
-- Mismo criterio que ya usan carga/pago. Para casino OFF el OR no cambia nada.
-- Las funciones son IDÉNTICAS a su última versión (cierre-unique-6 / fix-4 /
-- cierre-inmediato-3 / mejoras-2); SOLO cambia la línea del guard.
-- ═════════════════════════════════════════════════════════════════════════════

-- ── Helper: lee casino_deposit_enabled del tenant. Default OFF (sin fila o value
--    != 'true' → false). Espeja fn_caja_enabled.
create or replace function fn_casino_enabled(p_tenant uuid)
returns boolean
language sql
stable
as $$
  select coalesce((select value = 'true'
                     from settings
                    where key = 'casino_deposit_enabled' and tenant_id = p_tenant
                    limit 1), false);
$$;


-- ── 1) fn_cobrar_sueldo (última: migraciones-caja-cierre-inmediato-3.sql) ──────
...
declare
  ...
  v_mov_id uuid;
begin
  if not (fn_caja_enabled(p_tenant_id) or fn_casino_enabled(p_tenant_id)) then
  ...
  -- Guard: si el operador ya cerró su turno en el período vigente, no cobra.
  if exists (
...
-- ── 2) fn_verificar_descarga (última: migraciones-caja-mejoras-2.sql) ──────────
...
-- ── 3) fn_aplicar_descarga (última: migraciones-caja-cierre-inmediato-3.sql) ───
...
-- ── 4) fn_cerrar_turno (última: migraciones-caja-cierre-unique-6.sql) ──────────
...
-- ── 5) fn_acreditar_traspaso (última: migraciones-caja-fix-acreditar-destino-4.sql) ──
